...
export default function Footer({ element }: Props) {
  ...
  return (
    <div className="lg:hidden flex flex-col items-center justify-center border-b px-4 py-8 mb-5">
      ...
      <div className={`flex flex-row gap-x-3 items-center underline underline-offset-2 text-xs md:text-sm xl:text-md`}>
        <Link className='text-center' href={appLinks.returnPolicy.path}>{t('return_policy')}</Link>
        <Link className='text-center' href={appLinks.shippingPolicy.path}>{t('shipping_policy')}</Link>
        <Link className='text-center' href={appLinks.privacyPolicy.path}>{t('privacy_policy')}</Link>
      </div>
    </div>
  );
}
